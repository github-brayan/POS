...
class VistaPedido(tk.Frame):
    def __init__(self, parent, controller):
        ...
        # --- BARRA DE BÚSQUEDA ---
        self.frame_busqueda = tk.Frame(self.frame_productos, bg=Theme.COLOR_FONDO_SECUNDARIO)
        self.frame_busqueda.pack(fill='x', pady=(10,5), padx=10)
        tk.Label(self.frame_busqueda, text="Buscar:", font=Theme.FONT_BOTON, bg=Theme.COLOR_FONDO_SECUNDARIO).pack(side='left', padx=(0,5))
        self.search_var = tk.StringVar()
        self.search_var.trace_add("write", self.filtrar_productos)
        
        # --- FIX 2.1: Asignar el Entry de búsqueda a self.search_entry ---
        self.search_entry = tk.Entry(self.frame_busqueda, textvariable=self.search_var, font=Theme.FONT_NORMAL, relief="solid", bd=1)
        self.search_entry.pack(fill='x', expand=True, ipady=4)
        # --- FIX 2.2: Vincular eventos de foco para pausar/reanudar atajos ---
        self.search_entry.bind("<FocusIn>", self.on_search_focus_in)
        self.search_entry.bind("<FocusOut>", self.on_search_focus_out)
        
        self.canvas_prod = tk.Canvas(self.frame_productos, bg=Theme.COLOR_FONDO_SECUNDARIO, highlightthickness=0)
        self.canvas_prod.pack(side="left", fill="both", expand=True, padx=10, pady=(0,10))

        scrollbar_prod = ttk.Scrollbar(self.frame_productos, orient="vertical", command=self.canvas_prod.yview)
        scrollbar_prod.pack(side="right", fill="y", pady=(0,10), padx=(0,10))
        
        self.grid_productos = tk.Frame(self.canvas_prod, bg=Theme.COLOR_FONDO_SECUNDARIO)
        self.canvas_prod.create_window((0, 0), window=self.grid_productos, anchor="nw")
        self.canvas_prod.configure(yscrollcommand=scrollbar_prod.set)
        
        self.bind_all("<MouseWheel>", lambda e, c=canvas_cat, p=self.canvas_prod: self._on_mousewheel(e, c, p))
        self.grid_productos.bind("<Configure>", lambda e: self.canvas_prod.configure(scrollregion=self.canvas_prod.bbox("all")))
    ...
    # --- FIX 2.3: Nuevas funciones para manejar el foco de la búsqueda ---
    def on_search_focus_in(self, event):
        """ Se ejecuta cuando el usuario hace clic en la barra de búsqueda. """
        self.controller.unbind("<space>")
        self.controller.unbind("p")
        
    def on_search_focus_out(self, event):
        """ Se ejecuta cuando el usuario hace clic fuera de la barra de búsqueda. """
    ...
    def cargar_datos(self):
        ...
        # Asegurarse que la barra de búsqueda esté visible al cargar
        self.frame_busqueda.pack(fill='x', pady=(10, 5), padx=10, before=self.canvas_prod)
...
class VistaPago(tk.Frame):
    ...
    def finalizar_venta(self):
        if self.metodo_pago.get() == "Efectivo" and not self.paga_con_var.get():
            return
        ...
        try:
            ...
            for prod_id, item in self.orden_original['ticket'].items():
                real_prod_id = -1
                if 'var_' in str(prod_id): real_prod_id = int(str(prod_id).split('_')[1])
                else: real_prod_id = int(prod_id)
                cursor.execute(
            ...
            metodo_pago_final = self.metodo_pago.get()

            if metodo_pago_final == "Efectivo" and cambio > 0:
                print("DEBUG: Mostrando ventana de cambio.")
                VentanaCambio(self.controller, cambio)

            def acciones_post_venta():
                print(f"DEBUG: Post-venta. Método='{metodo_pago_final}', Cambio=${cambio:.2f}")
            ...
            self.controller.after(50, acciones_post_venta)
...
class VistaGestion(tk.Frame):
    ...
    def cargar_datos(self):
        self.cargar_categorias_listbox()
        self.cargar_productos_treeview()
    ...
    def cargar_categorias_listbox(self):
    ...
    def cargar_productos_treeview(self):
        for i in self.tree_productos.get_children(): self.tree_productos.delete(i)
        conn = conectar_db()
        cursor = conn.cursor()
        query = """
            SELECT p.id, p.nombre, p.precio, c.nombre, p.precio_variable
            FROM Productos p JOIN Categorias c ON p.id_categoria = c.id
            ORDER BY p.nombre
        """
        cursor.execute(query)
        for row in cursor.fetchall():
            es_variable_texto = "Sí" if row[4] else "No"
            self.tree_productos.insert("", "end", values=(row[0], row[1], f"{row[2]:.2f}", row[3], es_variable_texto))
        conn.close()
